...
def load_and_eval_ppl(model, model_name='', device=torch.device("cuda:0"), dataset='wikitext2', seqlen=2048, testloader=None, tokenizer=None):
    ...
    with torch.no_grad():
        ppl_test = eval_ppl(model, testloader, bs=1, seqlen=seqlen, device=device)
    return ppl_test 
...
def eval_metric(model, metric, loader, device, seqlen):
    if metric == 'ppl':
        return eval_ppl(model, loader, bs=1, seqlen=seqlen, device=device)
    elif metric == 'loss':
        return eval_loss(model, loader, bs=1, seqlen=seqlen, device=device)
    else:
