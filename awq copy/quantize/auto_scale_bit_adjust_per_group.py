...
@torch.no_grad()
def auto_scale_block_bit_adjust_per_group(module, module_kwargs, w_bit, q_config, input_feat, module_bit = None):
    ...
    def _search_module_scale_bit_adjust_per_group(block, linears2scale: list, x, kwargs={}, module_bit=None):
        # w: co, ci
        # x: n, ci
        assert module_bit is not None

        ## linears2scale 길이가 3이면 qkv, 2이면 ug, 1이면 o 혹은 down
        ## o 일 때, in_features=4096, out도 마찬가지
        ## down 일 때, in_features = 11008, out은 4096
        if len(linears2scale) == 3:
            bit = module_bit['self_attn.q_proj']
        elif len(linears2scale) == 2:
            bit = module_bit['mlp.up_proj']
        elif len(linears2scale) == 1:
            if block.in_features == 4096:
                bit = module_bit['self_attn.o_proj']
            elif block.in_features == 11008:
                bit = module_bit['mlp.down_proj']
        else:
    ...
    def _auto_get_scale(prev_op, layers, inp, module2inspect=None, kwargs={}):
        ...
        scales = _search_module_scale_bit_adjust_per_group(module2inspect, layers, inp, kwargs, module_bit)
