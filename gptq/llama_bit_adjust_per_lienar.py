...
@torch.no_grad()
def llama_sequential(model, dataloader, dev, arch):
    print('Starting ...')
    ...
    for i in range(len(layers)):
        ...
        for names in sequential:
            ...
            for name in subset:
                ...
                # )
                gptq[name].quantizer.configure(
                    arch['linear'][name][i], perchannel=True, sym=args.sym, mse=False
                )
...
if __name__ == '__main__':
    ...
    assert (args.arch_path is not None), "Please provide the architecture file for bit_adjust_per_linear"
    ...
    if args.wbits < 16 and not args.nearest:
        tick = time.time()
        quantizers = llama_sequential(model, dataloader, DEV, arch)
